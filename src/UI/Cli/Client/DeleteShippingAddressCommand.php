...
use App\Application\Command\Client\DeleteShippingAddress\DeleteShippingAddressCommand as DeleteAddressFromClientCommand;

class DeleteShippingAddressCommand extends Command
{
    use HandleTrait;
    use CastsTrait;
    ...
    {
        $this
            ->setName('client:delete-shipping-address')
            ->setDescription('Deleting shipping address from client.')
            ->setDefinition(
                new InputDefinition([
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->handle(new DeleteAddressFromClientCommand($uuid, $country, $city, $street, $zipCode));

        $output->writeln(
            '<info>Shipping address deleting from client successfully.</info>'
        );
