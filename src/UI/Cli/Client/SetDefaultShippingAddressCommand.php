...
use App\Application\Command\Client\SetDefaultShippingAddress\SetDefaultShippingAddressCommand
    as SetDefaultShippingAddressOnClientCommand;

class SetDefaultShippingAddressCommand extends Command
{
    use HandleTrait;
    use CastsTrait;
    ...
    {
        $this
            ->setName('client:set-default-shipping-address')
            ->setDescription('Set default shipping address to client.')
            ->setDefinition(
                new InputDefinition([
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->handle(new SetDefaultShippingAddressOnClientCommand($uuid, $country, $city, $street, $zipCode));

        $output->writeln(
            '<info>Shipping address set like DEFAULT on client successfully.</info>'
        );
